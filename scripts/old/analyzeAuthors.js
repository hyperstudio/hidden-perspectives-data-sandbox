...
const authorsJSON = require('./json/documents-authors.json');
...
authorsJSON.forEach((document, index) => {
	const { name } = document;
	const textToBeAnalyzed = name;

	if (index >= fromTo[0] && index < fromTo[1]) {
		// console.log(authorsJSON[index].documentOriginalID);
...
Promise.all(analyzedTextPromises).then((results) => {
	const documentEntities = results.map((result, index) => {
		// const mentionedIn = authorsJSON[index + fromTo[0]].documentOriginalID;
		const mentionedIn = authorsJSON[index + fromTo[0]].documentAuthor;
	...
	try {
		fs.writeFile(
			`./json/authors_entities__${fromTo[0]}-${fromTo[1]}.json`,
